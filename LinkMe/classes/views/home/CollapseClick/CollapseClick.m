...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        ...
        NSInteger hei = cell.ContentView.frame.size.height;
...
-(void)didSelectCollapseClickButton:(UIButton *)titleButton {
    ...
    // Cell is CLOSED -> OPEN
        ...
        //othe cell close
        NSArray *titleArray = @[@"0",@"1",@"2"];
        for(NSString *index in titleArray){
            if(titleButton.tag == [index intValue]){
                [self openCollapseClickCellAtIndex:[index intValue] animated:YES];
            }
            else{
                [self closeCollapseClickCellAtIndex:[index intValue] animated:NO];
            }
        }
        isOpen = YES;
    }
